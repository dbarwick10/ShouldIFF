...
import { FETCH_INTERVAL_MS, RETRY_INTERVAL_MS, USE_CUMULATIVE_STATS } from "./config/constraints.js"; 
...
export async function displayAverageEventTimes(averageEventTimes, calculateStats) {
    ...
    function renderAllCharts() {
            ...
            statKeys.forEach((key) => {
                ...
                if (data.length > 0) {
                    datasets.push({
                        ...
                        fill: false,
                        tension: 0.3,
                        pointRadius: data.length === 1 ? 2 : 1,
                        pointHoverRadius: data.length === 1 ? 2 : 1,
                        pointBackgroundColor: colorConfig[key].borderColor,
                        pointBorderWidth: 2,
                        order: 1
                    });
                }
            ...
            if (previousGameStats?.[currentCategory]) {
                ...
                if (dataToAdd.length > 0) {
                    datasets.push({
                        ...
                        pointRadius: data.length === 1 ? 2 : 1,
                        pointHoverRadius: data.length === 1 ? 2 : 1,
                        pointBackgroundColor: colorConfig.previousGame.borderColor,
                        pointBorderWidth: 2,
                        order: 2
                    });
                }
            ...
            if (currentLiveStats?.[currentCategory]) {
                ...
                if (dataToAdd.length > 0) {
                    datasets.push({
                        ...
                        fill: true,
                        tension: 0.3,
                        ppointRadius: data.length === 1 ? 2 : 1,
                        pointHoverRadius: data.length === 1 ? 2 : 1,
        ...
        return newCharts;
    } else if (USE_CUMULATIVE_STATS === false) {
            ...
            chartsToRender.forEach(stat => {
                ...
                statKeys.forEach((key) => {
                    ...
                    if (stat === 'deathTimers') {
                        if (categoryData.deaths?.length > 0 && categoryData.timeSpentDeadCumulative?.length > 0) {
                            data = categoryData.deaths
                                .map((deathTime, index) => ({
                                    x: deathTime / 60,
                                    y: categoryData.timeSpentDeadCumulative[index]
                                }))
                                .filter(point => point.x != null && point.y != null);
                        }
                    } else if (stat === 'kda') {
                        data = categoryData.kdaCumulative?.map(point => ({
                            x: point.timestamp / 60,
                            y: point.kdaValue
                        })) || [];
                    } else {
                        // Use cumulative arrays for all other stats
                        const cumulativeArray = categoryData[`${stat}Cumulative`];
                        const timeArray = categoryData[stat];
                        
                        if (Array.isArray(timeArray) && Array.isArray(cumulativeArray)) {
                            data = timeArray.map((time, index) => ({
                                x: time / 60,
                                y: cumulativeArray[index]
                            }));
                        }
                    }
                ...
                if (previousGameStats?.[currentCategory]) {
                    ...
                    if (stat === 'deathTimers') {
                        if (previousGameStats[currentCategory].deaths?.length > 0 && 
                            previousGameStats[currentCategory].timeSpentDeadCumulative?.length > 0) {
                            dataToAdd = previousGameStats[currentCategory].deaths
                                .map((deathTime, index) => ({
                                    x: deathTime / 60,
                                    y: previousGameStats[currentCategory].timeSpentDeadCumulative[index]
                                }))
                                .filter(point => point.x != null && point.y != null);
                        }
                    } else if (stat === 'kda') {
                        dataToAdd = previousGameStats[currentCategory].kdaCumulative?.map(point => ({
                            x: point.timestamp / 60,
                            y: point.kdaValue
                        })) || [];
                    } else {
                        const cumulativeArray = previousGameStats[currentCategory][`${stat}Cumulative`];
                        const timeArray = previousGameStats[currentCategory][stat];
                        
                        if (Array.isArray(timeArray) && Array.isArray(cumulativeArray)) {
                            dataToAdd = timeArray.map((time, index) => ({
                                x: time / 60,
                                y: cumulativeArray[index]
                            }));
                        }
                ...
                if (currentLiveStats?.[currentCategory]) {
                    ...
                    if (stat === 'deathTimers') {
                        if (currentLiveStats[currentCategory].deaths?.length > 0 && 
                            currentLiveStats[currentCategory].timeSpentDeadCumulative?.length > 0) {
                            dataToAdd = currentLiveStats[currentCategory].deaths
                                .map((deathTime, index) => ({
                                    x: deathTime / 60,
                                    y: currentLiveStats[currentCategory].timeSpentDeadCumulative[index]
                                }))
                                .filter(point => point.x != null && point.y != null);
                        }
                    } else if (stat === 'kda') {
                        dataToAdd = currentLiveStats[currentCategory].kdaCumulative?.map(point => ({
                            x: point.timestamp / 60,
                            y: point.kdaValue
                        })) || [];
                    } else {
                        const cumulativeArray = currentLiveStats[currentCategory][`${stat}Cumulative`];
                        const timeArray = currentLiveStats[currentCategory][stat];
                    ...
                    if (dataToAdd.length > 0) {
                        datasets.push({
                            ...
                            pointRadius: data.length === 1 ? 2 : 1,
                            pointHoverRadius: data.length === 1 ? 2 : 1,
                            pointBackgroundColor: colorConfig.live.borderColor,
                            pointBorderWidth: 2,
                            order: 3
                        });
                    }
                ...
                const chartOptions = {
                    ...
                    plugins: {
                        title: { 
                            display: true, 
                            text: stat === 'deathTimers' ? 'Cumulative Time Dead' : 
                                  (stat === 'kda' ? 'Average KDA' : `Average ${capitalizeFirstLetter(stat)}`) + ' Over Time',
                            font: {
                                size: 16,
                                weight: 'bold'
                            }
                        ...
                        tooltip: {
                            ...
                            callbacks: {
                                label: function(context) {
                                    ...
                                    if (stat === 'deathTimers') {
                                        return `${label}: ${value.toFixed(1)}s cumulative at ${timeFormatted}`;
                                    } else if (stat === 'kda') {
                                        return `${label}: ${value.toFixed(2)} average at ${timeFormatted}`;
                                    } else {
                                        return `${label}: ${value.toFixed(2)} average at ${timeFormatted}`;
                                    }
                                }
                    ...
                    scales: {
                        ...
                        y: { 
                            title: { 
                                display: true, 
                                text: stat === 'deathTimers' ? 'Cumulative Time Dead (Seconds)' : 
                                      stat === 'kda' ? 'Average KDA Ratio' : `Average ${capitalizeFirstLetter(stat)}`,
                                font: {
                                    weight: 'bold'
                                }
                            },
